...
def test_failing_roof_edge_case_5():
    """Test the translation of another room with a roof."""
    model_file = './tests/json/wall_roof_bug_3.dfjson'
    model = Model.from_file(model_file, cleanup_irrational=True)
    ...
    assert len(main_room.roof_ceilings) > 1
    assert main_room.geometry.is_solid
    assert len(main_room.walls) == 67


def test_failing_roof_edge_case_6():
    """Test the translation of another room with a roof."""
    model_file = './tests/json/wall_roof_bug_4.dfjson'
    model = Model.from_file(model_file, cleanup_irrational=True)
    ...
    assert len(main_room.roof_ceilings) > 1
    assert main_room.geometry.is_solid
    assert len(main_room.walls) == 5


def test_failing_roof_edge_case_7():
    """Test the translation of another room with a roof."""
    model_file = './tests/json/wall_roof_bug_5.dfjson'
    model = Model.from_file(model_file, cleanup_irrational=True)
...
def test_failing_roof_edge_case_8():
    """Test the translation of another room with a roof."""
    model_file = './tests/json/wall_roof_bug_6.dfjson'
    model = Model.from_file(model_file, cleanup_irrational=True)
...
def test_model_with_clearstory_windows():
    ...
    assert len(main_room.roof_ceilings) > 1
    assert main_room.geometry.is_solid
    assert len(main_room.apertures) in (2, 4)
...
def test_check_roofs_above_rooms():
    """Test the check_roofs_above_rooms method."""
    ...
    pts3 = (Point3D(0, 0, 3.25), Point3D(20, 0, 3.25), Point3D(20, 5, 5), Point3D(0, 5, 5))
    pts4 = (Point3D(0, 5, 5), Point3D(20, 5, 5), Point3D(20, 10, 3.25), Point3D(0, 10, 3.25))

    # Two rooms with different plenums heights
    room2d_full = Room2D(
    ...
    assert model.check_roofs_above_rooms(raise_exception=False) == ''
    assert model.check_roofs_above_rooms(raise_exception=False, detailed=True) == []
...
def test_check_for_error():
    ...
    assert model.check_for_error('100105', raise_exception=False) == ''
    assert model.check_for_error('100105', raise_exception=False, detailed=True) == []
    room2d_plenum.ceiling_plenum_depth = 0.5
    assert model.check_for_error('100105', raise_exception=False) != ''
    assert model.check_for_error('100105', raise_exception=False, detailed=True) != []
...
def test_to_honeybee_non_manifold_roof_issue():
    ...
    hb_models = model.to_honeybee('District', None, False, tolerance=0.001)
    assert len(hb_models) == 1
    assert len(hb_models[0].rooms[-1].roof_ceilings) > 1
