...
class _ClerestoryParameterBase(object):
    """Base object for all Clerestory parameters.

    This object records the minimum number of the methods that must be overwritten
    on a clerestory parameter object for it to be successfully be applied in
    dragonfly workflows.
    """
    ...
    def user_data(self, value):
        if value is not None:
            assert isinstance(value, dict), 'Expected dictionary for clerestory ' \
                'parameter user_data. Got {}.'.format(type(value))
        self._user_data = value

    def area_from_face(self, face):
        """Get the clerestory area generated by these parameters from a Room2D Face3D."""
        return 0

    def add_clerestory_to_face(self, face, tolerance=0.01):
        """Add Apertures to a Honeybee Roof Face using these Clerestory Parameters."""
        pass

    def scale(self, factor):
        """Get a scaled version of these ClerestoryParameters.
    ...
    def from_dict(cls, data):
        """Create ClerestoryParameterBase from a dictionary.

        .. code-block:: python

            {
            "type": "ClerestoryParameterBase"
            }
        """
        assert data['type'] == 'ClerestoryParameterBase', \
            'Expected ClerestoryParameterBase dictionary. Got {}.'.format(data['type'])
        return cls()

    def to_dict(self):
        """Get ClerestoryParameterBase as a dictionary."""
        return {'type': 'ClerestoryParameterBase'}
    ...
    def _add_user_data(self, new_win_par):
        """Add copies of this object's user_data to new ClerestoryParameters."""
    ...
    def _apply_user_data_to_honeybee(self, sub_faces, clean_data=None):
        """Apply the ClerestoryParameters user_data to generated honeybee objects.

        Args:
            sub_faces: An array of Honeybee Apertures or Doors to which user_data
                will be applied from this ClerestoryParameters.
            clean_data: An optional dictionary of user_data to be used in place
                ...
                ClerestoryParameters are able to be applied to the generated Honeybee
    ...
    def __copy__(self):
        return _ClerestoryParameterBase()

    def __repr__(self):
        return 'ClerestoryParameterBase'


class DetailedClerestory(_ClerestoryParameterBase):
    """Instructions for detailed clerestory windows, defined by 2D Polygons.

    Args:
        base_line: A ladybug_geometry LineSegment2D noting where on the floor
            plan of the model the clerestory window polygons are located.
            This establishes the plane and domain in which the clerestory
            geometries exist.
        elevation: A number for the Z-coordinate that places the base_line and
            the corresponding clerestory window polygons in 3D space. This
            elevation value should be below all of the 3D clerestory window
            geometries and sets the origin of the plane in which the
            clerestory geometries exist.
        polygons: An array of ladybug_geometry Polygon2D objects within the plane
            of the base_line with one polygon for each clerestory window. The base_line
        ...
        are_doors: An array of booleans that align with the polygons and note whether
            each of the polygons represents a door out onto a roof or balcony (True)
            or a clerestory window (False). If None, it will be assumed that
            all polygons represent windows and they will be translated
            to Apertures in any resulting Honeybee model. (Default: None).
    ...
    Properties:
        ...
        * base_plane
    """
    __slots__ = ('_base_line_3d', '_polygons', '_are_doors')

    def __init__(self, base_line, elevation, polygons, are_doors=None):
        """Initialize DetailedClerestory."""
        _ClerestoryParameterBase.__init__(self)  # add the user_data

        # process the base_line
        assert isinstance(base_line, LineSegment2D), \
            'Expected ladybug_geometry LineSegment2D. Got {}'.format(type(base_line))
        elevation = float_in_range(elevation, input_name='elevation')
        origin = Point3D(base_line.p.x, base_line.p.y, elevation)
        x_axis = Vector3D(base_line.v.x, base_line.v.y, 0)
        self._base_line_3d = LineSegment3D(origin, x_axis)
        ...
        for polygon in polygons:
            assert isinstance(polygon, Polygon2D), \
                'Expected Polygon2D for clerestory polygon. Got {}'.format(type(polygon))
        assert len(polygons) != 0, \
            'There must be at least one polygon to use DetailedClerestory.'
        ...
        if are_doors is None:
            ...
            for is_dr in are_doors:
                assert isinstance(is_dr, bool), 'Expected booleans for ' \
                    'DetailedClerestorys.are_doors. Got {}'.format(type(is_dr))
            assert len(are_doors) == len(polygons), \
                'Length of DetailedClerestory.are_doors ({}) does not match the length ' \
                'of DetailedClerestory.polygons ({}).'.format(
                    len(are_doors), len(polygons))
            self._are_doors = are_doors

    @classmethod
    def from_honeybee(cls, sub_faces):
        """Create DetailedClerestory from an array of Honeybee Apertures and Doors.

        Args:
            sub_faces: A list of Honeybee Apertures and/or Doors to be converted
                to Dragonfly DetailedClerestory. These Apertures and Doors
                must all be within the same plane for the resulting
                DetailedClerestory object to be valid.
        """
        ...
        for sf in sub_faces:
            verts2d = tuple(base_plane.xyz_to_xy(pt) for pt in sf.geometry.boundary)
            ...
            if sf.user_data is not None:
                for key, val in sf.user_data.items():
                    ...
                    try:
                        ...
                        user_dt[key] = [val]
        clear_par = cls(base_line, elevation, polygons, are_doors)
    ...
    def from_face3ds(cls, face3ds, are_doors=None):
        """Create DetailedClerestory from Face3Ds.

        Args:
            face3ds: A list of Face3D objects for the detailed clerestory windows.
    ...
    def _evaluate_face3d_base_plane(face3ds):
        """Get the base plane to be used to convert Face3Ds into clerestory polygons."""
    ...
    @property
    def base_line(self):
        """Get LineSegment2D that places the clerestory window polygons in plan."""
        bl3 = self._base_line_3d
        return LineSegment2D(Point2D(bl3.p.x, bl3.p.y), Vector2D(bl3.v.x, bl3.v.y))

    @property
    def elevation(self):
        """Get the Z-coordinate that places the base_line in 3D space."""
        return self._base_line_3d.p.z

    @property
    def polygons(self):
        """Get an array of Polygon2Ds with one polygon for each clerestory."""
    ...
    @property
    def base_line_3d(self):
        """Get LineSegment3D that places the clerestory window polygons in 3D space."""
        return self._base_line_3d

    @property
    def base_plane(self):
        """Get LineSegment3D that places the clerestory window polygons in 3D space."""
        origin, x_axis = self._base_line_3d.p2, self._base_line_3d.v
        normal = Vector3D(-x_axis.y, x_axis.x, 0)
        bp = Plane(n=normal, o=origin, x=x_axis)
        if bp.y.z < 0:
            bp = bp.rotate(bp.n, math.pi, bp.o)
        return bp

    def area(self):
        """Get the sub-face area generated by these parameters."""
        return sum(polygon.area for polygon in self._polygons)

    def aperture_area(self):
        """Get the Aperture area generated by these parameters."""
        return sum(poly.area for poly, isd in zip(self.polygons, self.are_doors)
                   if not isd)

    def overlapping_geometries(self, tolerance=0.01):
        """Get Face3D representing clerestory geometries that are overlapping.

        This is used to create helper_geometry for the case of invalid
        clerestory parameters.
        ...
        Returns:
            A list of Face3D representing overlapping clerestory geometries. This
        ...
        for poly_group in grouped_polys:
            if len(poly_group) > 1:
                for poly in poly_group:
                    pt3d = tuple(self.base_plane.xy_to_xyz(pt) for pt in poly)
                    overlap_geos.append(Face3D(pt3d))
        return overlap_geos

    def self_intersecting_geometries(self, tolerance=0.01):
        """Get Face3D representing clerestory geometries that are self-intersecting.
        ...
        Returns:
            A list of Face3D representing self-intersecting clerestory geometries.
        ...
        for polygon in self.polygons:
            if polygon.is_self_intersecting:
                ...
                if new_geo.is_self_intersecting:
                    pt3d = tuple(self.base_plane.xy_to_xyz(pt) for pt in polygon)
    ...
    def check_overlaps(self, tolerance=0.01):
        ...
        if not all(len(g) == 1 for g in grouped_polys):
            base_msg = '({} clerestory geometries overlap one another)'
            all_msg = []
    ...
    def check_self_intersecting(self, tolerance=0.01):
        """Check whether any polygons in these clerestory parameters are self intersecting.
        ...
        for i, polygon in enumerate(self.polygons):
            if polygon.is_self_intersecting:
                try:
                    ...
                    self_int_i.append(str(i))
        if len(self_int_i) != 0:
            return 'Clerestory polygons with the following indices are ' \
                'self-intersecting: ({})'.format(' '.join(self_int_i))
        return ''

    def check_valid_for_face(self, face):
        """Check that these clerestory parameters are valid for a given Face3D.
        ...
        Returns:
            A string with the message. Will be an empty string if valid.
        """
        # first check that the total clerestory area isn't larger than the roof
        total_area = face.area
        win_area = self.area_from_face(face)
        if win_area >= total_area:
            return 'Total area of clerestory windows [{}] is greater than the ' \
                'area of the parent wall [{}].'.format(win_area, total_area)
        # next, check to be sure that no clerestory is out of the roof boundary
        msg_template = 'Clerestory polygon {} is outside the range allowed ' \
            'by the parent wall.'
        verts2d = tuple(self.base_plane.xyz_to_xy(pt) for pt in face.boundary)
        parent_poly, parent_holes = Polygon2D(verts2d), None
        if face.has_holes:
            parent_holes = tuple(
                Polygon2D(self.base_plane.xyz_to_xy(pt) for pt in hole)
    ...
    def _remove_sub_faces(self, windows=False):
        ...
        if len(new_polygons) != 0:
            ...
            new_s_par = DetailedClerestory(new_polygons, are_doors)
    ...
    def add_clerestory_to_face(self, face, tolerance=0.01):
        """Add Apertures/Doors to a Honeybee Face using these Clerestory Parameters.
        ...
        """
        # get the polygons that represent the vertical face from the RoofSpecification
        base_plane = self.base_plane
        verts2d = tuple(base_plane.xyz_to_xy(pt) for pt in face.geometry.boundary)
        parent_poly, parent_holes = Polygon2D(verts2d), None
        if face.geometry.has_holes:
            parent_holes = tuple(
                Polygon2D(base_plane.xyz_to_xy(pt) for pt in hole)
        ...
        for i, (polygon, isd) in enumerate(zip(self.polygons, self.are_doors)):
            ...
            if poly_relation == -1:
                continue
            pt3d = tuple(base_plane.xy_to_xyz(p) for p in polygon)
            if None in pt3d:
                continue
            s_geos = [Face3D(pt3d)]
    ...
    def move(self, moving_vec):
        """Get this DetailedClerestory moved along a vector.

        Args:
            moving_vec: A Vector3D with the direction and distance to move the polygon.
        """
        new_c = self.duplicate()
        new_c._base_line_3d = self._base_line_3d.move(moving_vec)
        return new_c

    def scale(self, factor, origin=None):
        """Get a scaled version of this DetailedClerestory.
        ...
        """
        new_c = self.duplicate()
        new_c._base_line_3d = self._base_line_3d.scale(factor, origin)
        new_c._polygons = tuple(polygon.scale(factor) for polygon in self.polygons)
        return new_c

    def rotate(self, angle, origin):
        """Get this DetailedClerestory rotated counterclockwise in the XY plane.
        ...
        """
        new_c = self.duplicate()
        new_c._base_line_3d = self._base_line_3d.rotate_xy(math.radians(angle), origin)
        return new_c

    def reflect(self, plane):
        """Get a reflected version of this DetailedClerestory across a plane.

        Args:
            plane: A ladybug_geometry Plane across which the object will be reflected.
        """
        new_c = self.duplicate()
        new_c._base_line_3d = self._base_line_3d.reflect(plane.n, plane.o)
        return new_c

    def offset(self, offset_distance, tolerance=0.01):
        """Offset the edges of all clerestory polygons by a certain distance.
    ...
    def offset_polygons_for_face(self, face_3d, offset_distance=0.05, tolerance=0.01):
        ...
        Args:
            face_3d: A Face3D representing the vertical wall geometry of a
                RoofSpecification to which these clerestory parameters are assigned.
        ...
        # get the polygons that represent the vertical face from the RoofSpecification
        base_plane = self.base_plane
        verts2d = tuple(base_plane.xyz_to_xy(pt) for pt in face_3d.boundary)
        parent_poly, parent_holes = Polygon2D(verts2d), None
        ...
        for polygon, isd in zip(self.polygons, self.are_doors):
            if not self._is_sub_polygon(polygon, parent_poly, parent_holes):
                ...
                sub_face = Face3D([base_plane.xy_to_xyz(pt) for pt in polygon])
                bool_int = Face3D.coplanar_intersection(
                    face_3d, sub_face, tolerance, math.radians(1))
                if bool_int is None:  # clerestory completely outside parent
                    continue
                # offset the result of the boolean intersection from the edge
                    ...
                    for pt in new_f.boundary:
                        for edge in parent_edges:
                            ...
                            if pt.distance_to_point(close_pt) < offset_distance:
                                ...
                                pt = pt.move(move_vec)
                        new_pts_2d.append(base_plane.xyz_to_xy(pt))
    ...
    def make_flush(self, frame_distance, offset_boundary=False,
        ...
        """Make the edges of clerestory geometry flush if they lie within frame_distance.

        This is useful for translating between interfaces that expect the clerestory
        frame to be included within from the geometry.

        Args:
            frame_distance: Distance with which the edges of each clerestory window
                will be moved in order to make them flush with neighboring windows.
            offset_boundary: Boolean to note whether the outer boundary of clerestory
                ...
                clerestory geometries to account for the frame rather than just making
                the clerestory windows flush. (Default: True).
            tolerance: The minimum difference between point values for them to be
                considered the distinct. (Default: 0.01, suitable for objects
                in meters).
            angle_tolerance: The max angle difference in degrees that a clerestory
        ...
        # first group the clerestorys together if they lie within 2 times frame_distance
        ...
        # for each clerestory group, generate axes for aligning them flush
        flush_polys = []
        for ply_grp in grouped_polys:
            # if the group has only one clerestory, use the fast method
            if len(ply_grp) == 1:
                if offset_boundary:
            ...
            x_axes, _ = Polygon2D.common_axes(ply_grp, x_axis, min_distance,
                                              merge_distance, a_tol, None)
            # pull the clerestory polygons to the X and Y axes
    ...
    def rectangularize(self, percent_area_change_threshold=None):
        """Convert clerestory polygons into rectangles.
    ...
    def remove_self_intersecting(self, tolerance=0.01):
        """Get these clerestory parameters with self intersecting geometries removed.
        ...
        for i, (polygon, isd) in enumerate(zip(self.polygons, self.are_doors)):
            if not polygon.is_self_intersecting:
                ...
                try:
                    ...
                    pass
        # return the final clerestory parameters
        new_c_par = None
        if len(new_polygons) != 0:
            new_c_par = DetailedClerestory(
                self.base_line, self.elevation, new_polygons, new_are_doors
            )
        # update user_data lists if some windows were not added
        if new_c_par is not None and self.user_data is not None:
            clean_u = self.user_data
            if len(new_polygons) != len(self.polygons):
                clean_u = {}
                for key, val in self.user_data.items():
                    if isinstance(val, (list, tuple)) and len(val) >= len(self.polygons):
                        ...
                        clean_u[key] = val
            new_c_par.user_data = clean_u
        return new_c_par

    def union_overlaps(self, tolerance=0.01):
        """Union any clerestory polygons that overlap with one another.
    ...
    def merge_and_simplify(self, max_separation, tolerance=0.01):
        """Merge clerestory polygons that are close to one another into a single polygon.

        This can be used to create a simpler set of clerestory windows that is
        easier to edit and is in the same location as the original windows.

        Args:
            max_separation: A number for the maximum distance between clerestory polygons
                ...
                simply join neighboring clerestory windows together.
            tolerance: The maximum difference between point values for them to be
                considered distinct. (Default: 0.01, suitable for objects in meters).
        """
    ...
    def merge_to_bounding_rectangle(self, tolerance=0.01):
        """Merge clerestory polygons that touch or overlap with one another to a rectangle.
    ...
    def remove_duplicate_windows(self, tolerance=0.01):
        """Get a version of these clerestory parameters with duplicate geometries removed.
    ...
    def remove_small_windows(self, area_threshold):
        """Get a version of these clerestory parameters with small geometries removed.

        Args:
            area_threshold: A number for the area below which a clerestory polygon
                will be removed.
        """
        # evaluate the small clerestory windows
        new_polygons, new_are_doors, kept_i = [], [], []
        for i, (poly, is_dr) in enumerate(zip(self.polygons, self.are_doors)):
    ...
    @classmethod
    def from_dict(cls, data):
        """Create DetailedClerestory from a dictionary.
        ...
        """
        assert data['type'] == 'DetailedClerestory', \
            'Expected DetailedClerestory dictionary. Got {}.'.format(data['type'])
        are_doors = data['are_doors'] if 'are_doors' in data else None
        new_c_par = cls(
            LineSegment2D.from_array(data['base_line']),
            data['elevation'],
        ...
        if 'user_data' in data and data['user_data'] is not None:
            new_c_par.user_data = data['user_data']
        return new_c_par

    def to_dict(self):
        """Get DetailedClerestory as a dictionary."""
        base = {
            'type': 'DetailedClerestory',
            'base_line': self.base_line.to_array(),
    ...
    def __key(self):
        ...
        return (hash(self._base_line_3d),) + \
            tuple(hash(polygon) for polygon in self._polygons) + self.are_doors

    def __hash__(self):
        return hash(self.__key())

    def __eq__(self, other):
        return isinstance(other, DetailedClerestory) and \
            len(self._polygons) == len(other._polygons) and \
            self._base_line_3d == other._base_line_3d

    def __ne__(self, other):
        return not self.__eq__(other)

    def __repr__(self):
        return 'DetailedClerestory: [{} windows]'.format(len(self._polygons))
