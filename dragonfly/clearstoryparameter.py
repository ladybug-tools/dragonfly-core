...
class DetailedClearstory(_ClearstoryParameterBase):
    ...
    Args:
        base_line: A ladybug_geometry LineSegment2D noting where on the floor
            plan of the model the clearstory window polygons are located.
            This establishes the plane and domain in which the clearstory
            geometries exist.
        elevation: A number for the Z-coordinate that places the base_line and
            the corresponding clearstory window polygons in 3D space. This
            elevation value should be below all of the 3D clearstory window
        ...
        are_doors: An array of booleans that align with the polygons and note whether
            ...
            or a clearstory window (False). If None, it will be assumed that
    ...
    def __init__(self, base_line, elevation, polygons, are_doors=None):
        """Initialize DetailedClearstory."""
        _ClearstoryParameterBase.__init__(self)  # add the user_data
        ...
        for polygon in polygons:
            assert isinstance(polygon, Polygon2D), \
                'Expected Polygon2D for clearstory polygon. Got {}'.format(type(polygon))
        assert len(polygons) != 0, \
            'There must be at least one polygon to use DetailedClearstory.'
        ...
        if are_doors is None:
            ...
            for is_dr in are_doors:
                assert isinstance(is_dr, bool), 'Expected booleans for ' \
                    'DetailedClearstorys.are_doors. Got {}'.format(type(is_dr))
            assert len(are_doors) == len(polygons), \
                'Length of DetailedClearstory.are_doors ({}) does not match the length ' \
                'of DetailedClearstory.polygons ({}).'.format(
    ...
    def from_honeybee(cls, sub_faces):
        ...
        Args:
            sub_faces: A list of Honeybee Apertures and/or Doors to be converted
                to Dragonfly DetailedClearstory. These Apertures and Doors
        ...
        ref_plane = Plane(ref_plane.n, ref_plane.o)  # initializes with x in XY plane
        ...
        elevation = origin_3d.z
        seg_end_pt = Point3D(max_pt_3d.x, max_pt_3d.y, elevation)
        base_line = LineSegment2D.from_end_points(origin_3d, seg_end_pt)
        base_plane = Plane(n=ref_plane.n, o=origin_3d, x=base_line.v)
    ...
    def base_pane(self):
        """Get LineSegment3D that places the clearstory window polygons in 3D space."""
        origin, x_axis = self._base_line_3d.p, self._base_line_3d.v
        normal = Vector3D(-x_axis.x, x_axis.y, 0)
        return Plane(n=normal, o=origin, x=x_axis)
    ...
    def overlapping_geometries(self, tolerance=0.01):
        ...
        Returns:
            A list of Face3D representing overlapping clearstory geometries. This
    ...
    def self_intersecting_geometries(self, tolerance=0.01):
        ...
        Returns:
            A list of Face3D representing self-intersecting clearstory geometries.
    ...
    def check_overlaps(self, tolerance=0.01):
        ...
        if not all(len(g) == 1 for g in grouped_polys):
            base_msg = '({} clearstory geometries overlap one another)'
            all_msg = []
        ...
        for i, polygon in enumerate(self.polygons):
            if polygon.is_self_intersecting:
                try:
                    ...
                    self_int_i.append(str(i))
        if len(self_int_i) != 0:
            return 'Clearstory polygons with the following indices are ' \
    ...
    def check_valid_for_face(self, face):
        ...
        Returns:
            A string with the message. Will be an empty string if valid.
        """
        # first check that the total clearstory area isn't larger than the roof
        total_area = face.area
        win_area = self.area_from_face(face)
        if win_area >= total_area:
            return 'Total area of clearstorys [{}] is greater than the area of the ' \
                'parent roof [{}].'.format(win_area, total_area)
        # next, check to be sure that no clearstory is out of the roof boundary
        msg_template = 'Clearstory polygon {} is outside the range allowed ' \
            'by the parent roof.'
    ...
    def _remove_sub_faces(self, windows=False):
        ...
        if len(new_polygons) != 0:
            ...
            new_s_par = DetailedClearstory(new_polygons, are_doors)
    ...
    def move(self, moving_vec):
        """Get this DetailedClearstorys moved along a vector.
    ...
    def scale(self, factor, origin=None):
        """Get a scaled version of these DetailedClearstorys.
        ...
        """
        new_c = self.duplicate()
        new_c._base_line_3d = self._base_line_3d.move(factor, origin)
        return new_c

    def rotate(self, angle, origin):
        """Get these DetailedClearstorys rotated counterclockwise in the XY plane.
    ...
    def reflect(self, plane):
        """Get a reflected version of these DetailedClearstorys across a plane.
    ...
    def offset_polygons_for_face(self, face_3d, offset_distance=0.05, tolerance=0.01):
        ...
        Args:
            face_3d: A Face3D representing the vertical wall geometry of a
                RoofSpecification to which these clearstory parameters are assigned.
        ...
        for polygon, isd in zip(self.polygons, self.are_doors):
            if not self._is_sub_polygon(polygon, parent_poly, parent_holes):
                ...
                if bool_int is None:  # clearstory completely outside parent
                    continue
                # offset the result of the boolean intersection from the edge
    ...
    def make_flush(self, frame_distance, offset_boundary=False,
        ...
        This is useful for translating between interfaces that expect the clearstory
        frame to be included within from the geometry.

        Args:
            frame_distance: Distance with which the edges of each clearstory window
                will be moved in order to make them flush with neighboring windows.
            offset_boundary: Boolean to note whether the outer boundary of clearstory
                ...
                clearstory geometries to account for the frame rather than just making
                the clearstory windows flush. (Default: True).
            tolerance: The minimum difference between point values for them to be
                considered the distinct. (Default: 0.01, suitable for objects
                in meters).
            angle_tolerance: The max angle difference in degrees that a clearstory
        ...
        # first group the clearstorys together if they lie within 2 times frame_distance
        ...
        # for each clearstory group, generate axes for aligning them flush
        flush_polys = []
        for ply_grp in grouped_polys:
            # if the group has only one clearstory, use the fast method
            if len(ply_grp) == 1:
                if offset_boundary:
            ...
            x_axes, _ = Polygon2D.common_axes(ply_grp, x_axis, min_distance,
                                              merge_distance, a_tol, None)
            # pull the clearstory polygons to the X and Y axes
        ...
        for i, (polygon, isd) in enumerate(zip(self.polygons, self.are_doors)):
            if not polygon.is_self_intersecting:
                ...
                try:
                    ...
                    pass
        # return the final clearstory parameters
        new_c_par = None
        if len(new_polygons) != 0:
            new_c_par = DetailedClearstory(
    ...
    def merge_and_simplify(self, max_separation, tolerance=0.01):
        ...
        This can be used to create a simpler set of clearstory windows that is
        easier to edit and is in the same location as the original windows.

        Args:
            max_separation: A number for the maximum distance between clearstory polygons
                ...
                simply join neighboring clearstory windows together.
            tolerance: The maximum difference between point values for them to be
                considered distinct. (Default: 0.01, suitable for objects in meters).
        """
    ...
    def remove_small_windows(self, area_threshold):
        ...
        Args:
            area_threshold: A number for the area below which a clearstory polygon
                will be removed.
        """
        # evaluate the small clearstorys
        new_polygons, new_are_doors, kept_i = [], [], []
        for i, (poly, is_dr) in enumerate(zip(self.polygons, self.are_doors)):
    ...
    def from_dict(cls, data):
        ...
        """
        assert data['type'] == 'DetailedClearstory', \
            'Expected DetailedClearstory dictionary. Got {}.'.format(data['type'])
    ...
    def to_dict(self):
        """Get DetailedClearstory as a dictionary."""
        base = {
            'type': 'DetailedClearstory',
    ...
    def __copy__(self):
        new_s = DetailedClearstory(
            self._base_line, self._elevation, self._polygons, self._are_doors
        )
    ...
    def __key(self):
        ...
        return (hash(self._base_line), self._elevation) + \
    ...
    def __eq__(self, other):
        return isinstance(other, DetailedClearstory) and \
            len(self._polygons) == len(other._polygons) and \
            self._base_line == other._base_line and \
    ...
    def __repr__(self):
        return 'DetailedClearstory: [{} windows]'.format(len(self._polygons))
